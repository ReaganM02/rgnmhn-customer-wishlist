...
class WishlistAccountEndpoint {
	...
	public function hooks() {
		...
		add_action( 'woocommerce_account_menu_items', array( $this, 'addNewMenu' ), 99, 1 );

		// Add our endpoint to WooCommerce query vars.
		add_action( 'woocommerce_get_query_vars', array( $this, 'addQueryVar' ) );

		// Render page content when WooCommerce resolves this endpoint.
		add_action( 'woocommerce_account_' . $this->slug . '_endpoint', array( $this, 'pageContent' ) );

		// Internal hook: outputs the wishlist markup (used inside templates).
		add_action( 'rgnmhn_wishlist_my_account_content', array( $this, 'wishlistContent' ) );
	}
	 ...
	 * @return void
	 */
	private static function renderTemplateOnce( string $path, $data = array() ) {
	...
	public function assets() {
		...
		if ( is_account_page() && array_key_exists( MyAccountOptions::getSlug(), $wp->query_vars ) ) {
			...
			wp_enqueue_style( 'rgnmhn-my-account-wishlist', RGNMHN_CUSTOMER_WISHLIST_URL . 'assets/css/rgnmhn-my-account-wishlist.css', array(), RGNMHN_CUSTOMER_WISHLIST_VERSION );
			wp_enqueue_script( 'rgnmhn-my-account-wishlist-script', RGNMHN_CUSTOMER_WISHLIST_URL . 'assets/js/rgnmhn-customer-wishlist-my-account.js', array( 'jquery', 'wc-cart-fragments' ), RGNMHN_CUSTOMER_WISHLIST_VERSION, true );

			// Provide AJAX URL and nonces to the script.
			wp_localize_script(
				'rgnmhn-my-account-wishlist-script',
				'rgnmhn_wishlist_my_account',
				array(
